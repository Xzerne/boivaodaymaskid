	...
	function CheckQuest() 
        ...
        elseif World3 then
            ...
            elseif MyLevel == 2550 or MyLevel <= 2574 then
                ...
                NameMon = "Serpent Champion"
                CFrameQuest = CFrame.new(-16665.1914, 104.596405, 1579.69434, 0.877354681, 0, 0.479842514, 0, 1, 0, -0.479842514, 0, 0.877354681)
                CFrameMon = CFrame.new(-16521.0703, 105.592842, 1488.78198, 0.469467044, 0, 0.882950008, 0, 1, 0, -0.882950008, 0, 0.469467044)
            elseif MyLevel == 2550 or MyLevel <= 2574 then
                Mon = "Skull Slayer"
                LevelQuest = 2
                NameQuest = "TikiQuest3"
                ...
                CFrameQuest = CFrame.new(-16665.1914, 104.596405, 1579.69434, 0.877354681, 0, 0.479842514, 0, 1, 0, -0.479842514, 0, 0.877354681)
                CFrameMon = CFrame.new(-16855.0391, 120.957245, 1478.15698, -0.999391913, 0, -0.034868788, 0, 1, 0, 0.034868788, 0, -0.999391913)
            end
        end
    end
...
Module.Chests = setmetatable({}, {
    __call = function(self, ...)
        ...
        -- Kiểm tra và lấy danh sách các rương
        local Chests = {}
        ...
        for _, Chest in ipairs(Chests) do
            ...
            local Magnitude = (ChestPosition - Position).Magnitude
        ...
        if Nearest then
            ...
            while Nearest.Parent and _G.AutoFarmChest do
                local ChestPosition = Nearest:GetPivot().Position
                local PlayerPosition = HumanoidRootPart.Position

                if (ChestPosition - PlayerPosition).Magnitude <= 5000 + _G.MagnitudeAdd then
                    topos(Nearest.CFrame)

                    if (ChestPosition - PlayerPosition).Magnitude <= 1 then
                        EquipWeapon(_G.SelectWeapon)
                        wait(0.5)
                        game:GetService("VirtualUser"):CaptureController()
                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
    ...
    while wait() do
        if _G.ChestBypass then
            ...
            if Chest then
                ...
                if (Chest:GetPivot().Position - HumanoidRootPart.Position).Magnitude <= 5000 + _G.MagnitudeAdd then
                    repeat
                        wait()
                        if Chest.Parent then
                            TP3(Chest.CFrame)
                        end
                    until not _G.ChestBypass or not Chest.Parent
...
Module.EquipTool = setmetatable({}, {
    __call = function(self, Name, byTip)
        ...
        if Module.IsAlive(Char) then
            ...
            local ToolTip = (byTip and Name) or _G.SelectWeapon
...
local function ProcessEnemies(OthersEnemies, Folder)
    ...
    for _, Enemy in Folder:GetChildren() do
        ...
        if Head and Module.IsAlive(Enemy) and Player:DistanceFromCharacter(Head.Position) < FastAttack.Distance then
...
function FastAttack:Attack(BasePart, OthersEnemies)
    RegisterAttack:FireServer(_G.FastAttackDelay or 0.05)
...
function FastAttack:BladeHits()
    local Equipped = Module.IsAlive(Player.Character) and Player.Character:FindFirstChildOfClass("Tool")
