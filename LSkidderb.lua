...
spawn(function()
    while wait() do 
        if _G.AutoFarmChest then
            local chestFound = false -- Biến kiểm tra sự tồn tại của ChestModels
            for i, v in pairs(game:GetService("Workspace"):GetChildren()) do 
                if v.Name:find("ChestModels") then
                    chestFound = true -- Đánh dấu rằng có ít nhất 1 ChestModels tồn tại
            ...
            -- Nếu không tìm thấy bất kỳ "ChestModels" nào
...
function FastAttack:AttackNearest(ToolTip)
    ...
    if #OthersEnemies > 0 then
        if ToolTip == "Gun" then
            ...
            RegisterAttack:FireServer(Settings.ClickDelay or 0.01) -- Reduced delay
            RegisterHit:FireServer(Part1 or Part2, OthersEnemies)
        end
    else
        task.wait(0.1) -- Reduced idle wait time
    end
end
...
function FastAttack:BladeHits()
    ...
    if Equipped then
        ...
        task.wait(0.1) -- Reduced wait for next check
    end
end

task.spawn(function()
    while task.wait(_G.FastAttackDelay or 0.05) do -- Reduced delay between attacks
        if _G.FastAttack and not _G.AutoFarmFruitMastery and not _G.AutoFarmGunMastery and (tick() - Module.AttackCooldown) >= 0.1 then -- Reduced cooldown
